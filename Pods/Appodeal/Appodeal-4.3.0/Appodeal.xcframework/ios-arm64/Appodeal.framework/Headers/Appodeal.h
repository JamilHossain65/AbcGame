...
//
//  Copyright (c) 2026 Appodeal, Inc. All rights reserved.
//
...
/**
 Set custom Appodeal endpoint URL. Must be called before initialization.
 @note Objective-C
 <pre> [Appodeal setEndpoint:@"https://example.com/api"]; </pre>
 @note Swift
 <pre> Appodeal.setEndpoint("https://example.com/api") </pre>
 @param url - NSString value, must be nonnull
 */
+ (void)setEndpoint:(NSString *_Nonnull)url;
/**
 Get the current Appodeal endpoint URL
 @note Objective-C
 <pre>
...
/**
 Request only non-personalized advertising for this user. By default this setting is false.
 When enabled, the SDK limits the personalized data it sends. A publisher-set value takes
 precedence over the CMP.
 @param nonPersonalized Boolean flag indicating that the user should receive non-personalized ads.
 */
+ (void)setNonPersonalized:(BOOL)nonPersonalized;
/**
 Get framework type
 @note Objective-C
